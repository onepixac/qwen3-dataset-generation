...
class ChatGenerator:
    """
    Generates function RAG conversations from scientific textbook chunks
    ...
    def create_prompt(self, chunk_text: str, chunk_index: int) -> str:
        """Create function generation prompt"""
        domain_context = {
...
"""
    ...
    async def generate_batch(self, chunks: List[Dict], start_idx: int, end_idx: int) -> List[Dict]:
        """Generate function examples for a batch of chunks (PARALLEL)"""
        results = []
    ...
    def run(self):
        ...
        print(f"\n💬 FUNCTION GENERATOR - {self.materia.upper()}")
