...
class ChatGenerator:
    """
    Generates reasoning RAG conversations from scientific textbook chunks
    ...
    def create_prompt(self, chunk_text: str, chunk_index: int) -> str:
        """Create reasoning generation prompt"""
        domain_context = {
...
"""
    ...
    async def generate_batch(self, chunks: List[Dict], start_idx: int, end_idx: int) -> List[Dict]:
        """Generate reasoning examples for a batch of chunks (PARALLEL)"""
        results = []
        
        def process_chunk(idx):
            """Process single chunk (for parallel execution)"""
            chunk = chunks[idx]
        ...
        batch_results = [None] * (min(end_idx, len(chunks)) - start_idx)
        with ThreadPoolExecutor(max_workers=15) as executor:
            future_to_idx = {executor.submit(process_chunk, idx): idx - start_idx 
                             for idx in range(start_idx, min(end_idx, len(chunks)))}
            
            for future in as_completed(future_to_idx):
                result_idx = future_to_idx[future]
                batch_results[result_idx] = future.result()
    ...
    def run(self):
        ...
        print(f"\n💬 REASONING GENERATOR - {self.materia.upper()}")
