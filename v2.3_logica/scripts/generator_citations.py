...
class ChatGenerator:
    """
    Generates citations RAG conversations from scientific textbook chunks
    ...
    def create_prompt(self, chunk_text: str, chunk_index: int) -> str:
        """Create citations generation prompt"""
        domain_context = {
...
"""
    ...
    async def generate_batch(self, chunks: List[Dict], start_idx: int, end_idx: int) -> List[Dict]:
        """Generate citations examples for a batch of chunks"""
        results = []
        batch_results = []
    ...
    def run(self):
        ...
        print(f"\n💬 CITATIONS GENERATOR - {self.materia.upper()}")
