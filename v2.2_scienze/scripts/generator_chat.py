...
class ChatGenerator:
    """
    Generates chat RAG conversations from scientific textbook chunks
    ...
    def __init__(self, materia: str, target: int):
        ...
        self.output_file = self.base_dir / "output" / f"{materia}_chat.jsonl"
        self.checkpoint_file = self.base_dir / "checkpoints" / f"{materia}_chat_checkpoint.json"
        self.chunks_file = self.base_dir / "checkpoints" / f"{materia}_chunks.json"
        
        self.client = OpenRouterClient()
        self.completed = 0
        self.batch_size = 250  # LEVEL 9
        self.max_concurrent = 200  # LEVEL 9 (increased from 150)
...
"""
    ...
    async def generate_batch(self, chunks: List[Dict], start_idx: int, end_idx: int) -> List[Dict]:
        """Generate chat examples for a batch of chunks"""
        results = []
        ...
        for idx in range(start_idx, min(end_idx, len(chunks))):
            ...
            tasks.append(self.client.generate_json(prompt, max_retries=3))
        ...
        # Wait for all tasks (use synchronous client)
        batch_results = []
        for task in tasks:
            try:
                result = self.client.generate_json(
                    prompt=self.create_prompt(chunks[start_idx + len(batch_results)].get("text", ""), start_idx + len(batch_results)),
                    max_retries=3
                )
    ...
    def run(self):
        ...
        print(f"\n💬 CHAT GENERATOR - {self.materia.upper()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--materia", required=True, choices=["biologia", "chimica", "matematica_fisica", "logica"])
