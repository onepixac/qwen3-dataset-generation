"""Reasoning Generator v2.2 SCIENZE - LEVEL 9: 220 concurrent"""
...
class ReasoningGenerator:
    def __init__(self, materia, target):
        ...
        self.output_file = self.base_dir / "output" / f"{materia}_reasoning.jsonl"
        self.checkpoint_file = self.base_dir / "checkpoints" / f"{materia}_reasoning_checkpoint.json"
        self.chunks_file = self.base_dir / "checkpoints" / f"{materia}_chunks.json"
        self.client = OpenRouterClient()
        self.completed = 0
        self.max_concurrent = 220
    ...
    def create_prompt(self, text):
        return f"""Genera 2 reasoning questions da: {text[:2000]}\nJSON: [{{"question": "Perché/Come...", "evaluation": "Feedback pedagogico 150 parole con domande socratiche"}}]"""
    ...
    def generate_batch(self, chunks, start, end):
        ...
        for idx in range(start, min(end, len(chunks))):
            try:
                ...
                for item in data:
                    if "question" in item:
                        results.append({"messages": [{"role": "user", "content": item["question"]}, {"role": "assistant", "content": item.get("evaluation", "")}], "metadata": {"domain": self.materia, "task_type": "reasoning"}})
            except: pass
        return results
    
    def run(self):
        print(f"\n🤔 REASONING - {self.materia.upper()}, Target: {self.target}")
...
if __name__ == "__main__":
    ...
    ReasoningGenerator(args.materia, args.target).run()
