"""Function Calling Generator v2.2 SCIENZE - LEVEL 9: 150 concurrent"""
import json, argparse
from pathlib import Path
from tqdm import tqdm
import asyncio
from openrouter_client import OpenRouterClient

class FunctionGenerator:
    def __init__(self, materia, target):
        self.materia, self.target = materia, target
        self.base_dir = Path(__file__).parent
        self.output_file = self.base_dir / "output" / f"{materia}_function.jsonl"
        self.checkpoint_file = self.base_dir / "checkpoints" / f"{materia}_function_checkpoint.json"
        self.chunks_file = self.base_dir / "checkpoints" / f"{materia}_chunks.json"
        self.client = OpenRouterClient()
        self.completed = 0
        self.max_concurrent = 150
    
    def load_chunks(self):
        with open(self.chunks_file, encoding="utf-8") as f:
            return json.load(f)
    
    def load_checkpoint(self):
        if self.checkpoint_file.exists():
            with open(self.checkpoint_file) as f:
                return json.load(f).get("last_chunk_idx", 0)
        return 0
    
    def save_checkpoint(self, idx, count):
        with open(self.checkpoint_file, "w") as f:
            json.dump({"last_chunk_idx": idx, "examples_completed": count}, f)
    
    def create_prompt(self, text):
        return f"""Genera 1 function calling example da: {text[:1500]}\nJSON: [{{"query": "Richiesta tool", "tool": "plotting_tool/calculator", "params": {{}}, "result": "Risposta"}}]"""
    
    def generate_batch(self, chunks, start, end):
        results = []
        for idx in range(start, min(end, len(chunks))):
            try:
                text = chunks[idx].get("text", "")
                if not text: continue
                data = self.client.generate_json(self.create_prompt(text), max_retries=3)
                for item in data:
                    if "query" in item:
                        results.append({"messages": [{"role": "user", "content": item["query"]}, {"role": "assistant", "content": json.dumps({"tool": item.get("tool"), "params": item.get("params", {})})}], "metadata": {"domain": self.materia, "task_type": "function"}})
            except: pass
        return results
    
    def run(self):
        print(f"\n🎯 FUNCTION - {self.materia.upper()}, Target: {self.target}")
        chunks = self.load_chunks()
        start_idx = self.load_checkpoint()
        pbar = tqdm(total=self.target, initial=self.completed)
        idx = start_idx
        while self.completed < self.target and idx < len(chunks):
            batch_end = min(idx + 50, len(chunks))
            examples = self.generate_batch(chunks, idx, batch_end)
            if examples:
                with open(self.output_file, "a", encoding="utf-8") as f:
                    for ex in examples:
                        f.write(json.dumps(ex, ensure_ascii=False) + "\n")
                self.completed += len(examples)
                pbar.update(len(examples))
            if self.completed % 50 == 0:
                self.save_checkpoint(idx, self.completed)
            idx = batch_end
        pbar.close()
        self.save_checkpoint(idx, self.completed)
        print(f"✅ DONE: {self.completed}/{self.target}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--materia", required=True)
    parser.add_argument("--target", type=int, required=True)
    args = parser.parse_args()
    FunctionGenerator(args.materia, args.target).run()
