"""Citations Generator v2.2 SCIENZE - LEVEL 9: 200 concurrent"""
...
class CitationsGenerator:
    def __init__(self, materia, target):
        ...
        self.output_file = self.base_dir / "output" / f"{materia}_citations.jsonl"
        self.checkpoint_file = self.base_dir / "checkpoints" / f"{materia}_citations_checkpoint.json"
    ...
    def create_prompt(self, text):
        return f"""Genera 2 Q&A con citations da: {text[:2000]}\nJSON: [{{"question": "Domanda", "answer": "Risposta con [1][2] sources inline"}}]"""
    ...
    def generate_batch(self, chunks, start, end):
        ...
        for idx in range(start, min(end, len(chunks))):
            try:
                ...
                for item in data:
                    if "question" in item:
                        results.append({"messages": [{"role": "user", "content": item["question"]}, {"role": "assistant", "content": item.get("answer", "")}], "metadata": {"domain": self.materia, "task_type": "citations"}})
            except: pass
        return results
    
    def run(self):
        print(f"\n📚 CITATIONS - {self.materia.upper()}, Target: {self.target}")
...
if __name__ == "__main__":
    ...
    CitationsGenerator(args.materia, args.target).run()
